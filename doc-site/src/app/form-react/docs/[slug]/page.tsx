...
import { getFormReactNavItemByHref, getFormReactSectionByHref } from '@/lib/navigation-form-react';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    const documentation = await getDocumentation(params.slug, 'form-react');
    ...
    return {
        title: `${documentation.title} | qortex-form-react`,
        description: documentation.description || `Documentation for ${documentation.title}`,
    };
}

export default async function FormReactDocPage({ params }: PageProps) {
    const href = `/form-react/docs/${params.slug}`;
    const item = getFormReactNavItemByHref(href);
    const section = getFormReactSectionByHref(href);
    const documentation = await getDocumentation(params.slug, 'form-react');
