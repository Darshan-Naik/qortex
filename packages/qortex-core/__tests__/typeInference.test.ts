import { Fetcher, QueryKey } from "../src/queryManager/types";
import { queryManager } from "../src/queryManager";

// Destructuring works because all public methods are arrow functions
const { registerFetcher, getQueryData, getQueryState, dangerClearCache } = queryManager;
...
describe("Enhanced Type Inference Tests", () => {
  ...
  describe("Automatic Type Inference", () => {
    ...
    test("should work with explicit types when provided", () => {
      ...
      registerFetcher<{ id: number; name: string }>(["object-key"], {
      ...
      const objectData = getQueryData<{ id: number; name: string }>([
        "object-key",
      ]);
    ...
    test("should infer types correctly for getQueryState", () => {
      const fetcher: Fetcher<{ id: number; name: string }> = async () => ({
        id: 1,
        name: "test",
      });
