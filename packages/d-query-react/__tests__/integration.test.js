const { queryManager } = require('d-query');
const { subscribeToKey } = require('../dist/index.js');

describe('React Package Integration Tests', () => {
  let mockFetcher;
  ...
  describe('subscribeToKey Integration', () => {
  ...
  describe('Query State Integration', () => {
    ...
    test('should handle placeholderData option', async () => {
      ...
      // Subscribe with placeholderData
      const unsubscribe = subscribeToKey(['test-key'], callback, { 
        enabled: true,
        placeholderData
      });
      ...
      const state = queryManager.getQueryState(['test-key'], { enabled: false });
      expect(state.data).toEqual({ id: 1, data: 'test-data' });
