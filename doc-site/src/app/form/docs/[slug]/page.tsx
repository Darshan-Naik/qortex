...
import { getFormNavItemByHref, getFormSectionByHref } from '@/lib/navigation-form';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    const documentation = await getDocumentation(params.slug, 'form');
    ...
    return {
        title: `${documentation.title} | qortex-form`,
        description: documentation.description || `Documentation for ${documentation.title}`,
    };
}

export default async function FormDocPage({ params }: PageProps) {
    const href = `/form/docs/${params.slug}`;
    const item = getFormNavItemByHref(href);
    const section = getFormSectionByHref(href);
    const documentation = await getDocumentation(params.slug, 'form');
