...
import { getStoreReactNavItemByHref, getStoreReactSectionByHref } from '@/lib/navigation-store-react';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    const documentation = await getDocumentation(params.slug, 'qortex-store-react');
    ...
    return {
        title: `${documentation.title} | qortex-store-react`,
        description: documentation.description || `Documentation for ${documentation.title}`,
    };
}

export default async function StoreReactDocPage({ params }: PageProps) {
    const href = `/qortex-store-react/docs/${params.slug}`;
    const item = getStoreReactNavItemByHref(href);
    const section = getStoreReactSectionByHref(href);
    const documentation = await getDocumentation(params.slug, 'qortex-store-react');
